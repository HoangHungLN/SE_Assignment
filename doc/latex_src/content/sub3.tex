...
\textbf{Schedule / Session UI (gói giao diện buổi học)}  \\
...
\textbf{Report UI (gói giao diện báo cáo và phản hồi)}  \\
...
\textbf{Authentication Service (gói dịch vụ xử lý đăng nhập)}  \\
...
\textbf{Group Service (gói dịch vụ quản lý nhóm hướng dẫn)}  \\
...
\textbf{Session Register Service (gói dịch vụ quản lý đăng ký buổi học)}  \\
Xử lý logic cho việc "Mở buổi học" của tutor, "Tham gia buổi học" của sinh viên và thông báo về lịch học.

\textbf{Session Management Service (gói dịch vụ quản lý buổi học)}  \\
...
\textbf{Report Service (gói dịch vụ báo cáo cho các phòng ban quản lý)}  \\
...
\textbf{Entities (gói các thực thể)}  \\
Chứa các lớp đối tượng (entity) được lưu trữ, đại diện cho các thực thể chính của hệ thống.

\textbf{Data Service (gói các dịch vụ tương tác với thực thể)}  \\
...
\textbf{SSO Adapter (gói logic giao tiếp với HCMUT\_SSO)}  \\
Cung cấp phương thức để Business Layer gọi đến HCMUT\_SSO cho nhiệm vụ xác thực và phân quyền.

\textbf{Data Core Adapter (gói logic giao tiếp với HCMUT\_DATACORE)}  \\
...
\textbf{Library Adapter (gói logic giao tiếp với HCMUT\_LIBRARY)}  \\
...
\begin{figure}[H]
  ...
  \adjincludegraphics[
    ...
    trim={{0\width} {0\height} {0\width} {0\height}},
    clip
  ]{graphics/sub3/Class diagram_v2.png}     % ← thay bằng đường dẫn thực tế của ảnh
