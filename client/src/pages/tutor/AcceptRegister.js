...
const AcceptRegister = () => {
  const [requestedGroups, setRequestedGroups] = useState([]);   // cột trái: tutor = current, status = 'waiting'
  const [unassignedGroups, setUnassignedGroups] = useState([]); // cột phải: tutor = null
  const [selectedGroup, setSelectedGroup] = useState(null);
  const [selectedType, setSelectedType] = useState(null);     // "requested" | "accepted"
  ...
  const loadGroups = async () => {
    try {
      ...
      groups.forEach((g) => {
        ...
        // CỘT TRÁI:
        // Nhóm đã request tutor hiện tại, nhưng vẫn đang chờ quyết định
        // → tutor = CURRENT_TUTOR_ID và status = 'waiting'
        if (tutor && tutorID === CURRENT_TUTOR_ID && g.status === "waiting") {
          requested.push(g);
        }

        // CỘT PHẢI:
        // Tất cả nhóm chưa được hướng dẫn → tutor = null
        if (!tutor) {
          unassigned.push(g);
        }
      });
  ...
  const handleAccept = async (groupID) => {
    try {
      ...
      await loadGroups(); // reload lại 2 cột
    } catch (err) {
  ...
  const handleAcceptUnassigned = async (groupID) => {
    try {
      ...
      // Reload lại 2 cột cho đúng dữ liệu
      await loadGroups();
    } catch (err) {
        ...
        <div className="accept-group-columns">
          ...
          <section className="panel panel--requested">
              ...
              {requestedGroups.length === 0 ? (
                ...
                requestedGroups.map((group) => (
                  ...
                  >
                    ...
                    {/* Chặn click ở vùng nút để không mở modal */}
                    <div
            ...
            <footer className="panel-footer">
              ...
              {/* Placeholder phân trang nếu sau này cần */}
              <div className="pagination">
                <span className="pagination-box">&lt; 1/1 &gt;</span>
              </div>
            </footer>
          </section>
          ...
          <section className="panel panel--available">
            ...
            <div className="group-list group-list--grid">
              {unassignedGroups.length === 0 ? (
                ...
                unassignedGroups.map((group) => (
                  ...
                  >
                    ...
                    {/* Action: tutor nhận nhóm chưa có tutor */}
                    <div
                      className="group-card__actions"
            ...
            >
              ...
              <div className="group-modal__body">
                ...
                <div style={{marginTop: '15px'}}>
                    <div className="roup-modal__label" style={{marginBottom: '8px'}}>Thành viên:</div>
